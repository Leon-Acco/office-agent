"""
数据库种子数据填充
首次启动时将 Figma 设计稿中的示例数据写入 MySQL
"""
import uuid
...
async def run_seed():
    """执行种子数据填充（幂等：已有数据则跳过）"""
    # 种子数据开关：设置为 False 后不再填充假数据
    import os
    if os.environ.get("SEED_DATA", "false").lower() != "true":
        print("[seed] 种子数据已禁用（设置 SEED_DATA=true 启用）")
        return
    ...
    async with async_session() as session:
        ...
        for (title, domain, icon, status, owner, conf, pub_date, warning) in knowledge_data:
            session.add(KnowledgeCandidate(
                id=uuid.uuid4().hex,
                title=title,
                domain=domain,
